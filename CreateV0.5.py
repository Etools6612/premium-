...
try:
    ...
    pass
#──────────────{ FILE FOLDER }──────────────#
folder_path = '/sdcard/AUTO-CREATE-FILE/create/'
try:
...
[green1] /$$$$$$$$ /$$$$$$$   /$$$$$$         /$$$$$$  /$$$$$$$  /$$$$$$$$  /$$$$$$  /$$$$$$$$ /$$$$$$$$
| $$_____/| $$__  $$ /$$__  $$       /$$__  $$| $$__  $$| $$_____/ /$$__  $$|__  $$__/| $$_____/
| $$      | $$  \ $$| $$  \ $$      | $$  \__/| $$  \ $$| $$      | $$  \ $$   | $$   | $$      
| $$$$$   | $$$$$$$ | $$$$$$$$      | $$      | $$$$$$$/| $$$$$   | $$$$$$$$   | $$   | $$$$$   
[dark_olove_green]| $$__/   | $$__  $$| $$__  $$      | $$      | $$__  $$| $$__/   | $$__  $$   | $$   | $$__/   
| $$      | $$  \ $$| $$  | $$      | $$    $$| $$  \ $$| $$      | $$  | $$   | $$   | $$      
[pale_green]| $$      | $$$$$$$/| $$  | $$      |  $$$$$$/| $$  | $$| $$$$$$$$| $$  | $$   | $$   | $$$$$$$$
|__/      |_______/ |__/  |__/       \______/ |__/  |__/|________/|__/  |__/   |__/   |________/
...
""")
ll=str([hari,tanggal])
hx=("""  [bold green1]DEVELOPER[medium_purple1]   ⟩[cyan][bold] Admin
  [bold green1]FACEBOOK[medium_purple1]    ⟩[cyan][bold] admin 
  [bold green1]GITHUB[medium_purple1]      ⟩[bright_yellow] automatic api
  [bold green1]STATUS[medium_purple1]      ⟩[bright_yellow] ALL NETWORK 
  [bold green1]TOOLS[medium_purple1]       ⟩[bright_yellow] PREMIUM 
  [bold green1]UPDATES[medium_purple1]     ⟩[bright_yellow] September 
...
def AUTO_BRYX():
    ...
    if Bryx in ["a","A","1","01"]:
        ...
        os.system("xdg-open https://www.facebook.com/profile.php?id=61580871672001")
...
def main() -> None:
    ...
    for _ in range(int(num_accounts)):
        ...
        a.add("[violet][[yellow2]●[violet]] [bold green]NUMBER   [cyan2] ⟩ [bold yellow]"+phone2)
        a.add("[violet][[yellow2]●[violet]] [bold green]EMAIL    [cyan2] ⟩ [bold red]"+email2)
        payload = {
            'ccp': "2",
...
def confirm_id(mail,uid,otp,data,ses):
    try:
        ...
        if "checkpoint" in str(response.url):
            ...
            print(c)
            open("/sdcard/AUTO-CREATE-FILE/create/auto-create-ok.txt", "a").write(uid+"|"+otp+f"|{password}|"+mail+"|"+cookie+"\n")
            oks.append(uid)
    except Exception as e:    
        pass
#──────────────{ PROGRES }──────────────#
def progres(current, num_accounts, delay):
		for sleep in range(int(num_accounts), 0, -1):
			print(f'[creating]-[{current}|{num_accounts}]-[SUCCESS:-{len(oks)}|BAD:-{len(cps)}]',end='\r')
			time.sleep(1)
...
def register_facebook_account(password, first_name, last_name, birthday):
    ...
    if id:
        ...
        if 'Locked' in check:
            ...
            print(Panel(' [bold yellow]ACCOUNT ACCESSABLE LOADING PLEASE WAIT',style="bold violet"))
            time.sleep(30)
            try:
                cod = Email(em["session"]).inbox()
            ...
            if code:
                a=Tree(":file_folder:",guide_style="bold green_yellow")
                a.add(f"[violet][[yellow2]●[violet]] [bold green]NAME     [cyan2] ⟩ [bold red]{first_name} {last_name}")
                a.add("[violet][[yellow2]●[violet]] [bold green]EMAIL    [cyan2] ⟩ [bold red]"+email)
                a.add("[violet][[yellow2]●[violet]] [bold green]NUMBER   [cyan2] ⟩ [bold red]"+number)
                a.add("[violet][[yellow2]●[violet]] [bold green]LOGIN OTP[cyan2] ⟩ [bold red]"+code)
                ...
                Bryxc = Panel(f"[bold green] {password}", title="[bold red]PASS",width=30,padding=0,style="bold violet")
                Bryxe = Columns([Bryxa])
                Bryxf = Columns([Bryxb, Bryxc])
                c=Tree(":file_folder:",guide_style="bold green_yellow")
                c.add(Bryxe)
