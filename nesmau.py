...
def colorful_loading_bar(total=100, bar_length=40):
    colors = [Fore.RED, Fore.GREEN, Fore.YELLOW, Fore.CYAN, Fore.MAGENTA, Fore.BLUE]
    
    for i in range(total + 1):
        # pumili ng random na kulay bawat update
        color = random.choice(colors)
        
        # bilang ng blocks na puno
        ...
        sys.stdout.flush()
        time.sleep(0.05)  # bilis ng loading animation
...
def get_public_ip():
    try:
        ...
        return "Public IP not detected"


# ==========================
#  Security Check (IP Format)
# ==========================
def is_ip_secured(ip):
    pattern = r"^\d{1,3}(\.\d{1,3}){3}$"    # Basic IPv4 validation
...
print("\n--- SYSTEM IP INITIALIZATION ---")
print(f"Local IP  : {local_ip}")
print(f"Public IP : {public_ip}")

# Security validation
print("\n--- IP SECURITY STATUS ---")
print("Local IP secured  ✔" if is_ip_secured(local_ip) else "Local IP not valid ❗")
print("Public IP secured ✔" if is_ip_secured(public_ip) else "Public IP not valid ❗")

print("\nSystem IP initialization completed.")
...
def get_info():
    try:
        # Kunin IP address mula sa ipinfo.io
        ip_data = requests.get("https://ipinfo.io/json").json()
        ip_address = ip_data.get("ip", "N/A")

        # Kunin device/phone model (basic)
        phone_model = platform.platform()

        # Oras at petsa ngayon
        login_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

        print("📱 Device Login Info")
        print("-" * 40)
...
╚═════╝  ╚═════╝  ╚═════╝ ╚══════╝   ╚═╝   
 {Q7}─────────────────────────────────────────────────────────────────\033[0m
 {Q7}                 DEVELOPER KRISTEL
 {Q7}─────────────────────────────────────────────────────────────────\033[0m
 {Q7}TOOL OWNER   : KRISTEL 
 {Q7}TOOL TYPE    : AUTOMATIC BOOSTING TOOL
 {Q7}TOOL VERSION : {green}8.8.1.1
 {Q7}─────────────────────────────────────────────────────────────────\033[0m""")
...
def axl2():
    ...
    print(f"     {yellow}[01] {green}FRA EXTRACT ACCOUNT")
    print(f"     {yellow}[02] {green}FRA EXTRACT PAGES")
    print(f"     {yellow}[03] {green}RPW EXTRACT ACCOUNT")
    print(f"     {yellow}[04] {green}RPW EXTRACT PAGES")
    print(f" {Q7}─────────────────────────────────────────────────────────────────\033[0m")
    ...
    print(f" {Q7}─────────────────────────────────────────────────────────────────\033[0m")
    file_path = input(f" {yellow}PUT FILE PATH ( /storage/emulate/0/fb.txt ): ").strip()
    print(f" {Q7}─────────────────────────────────────────────────────────────────\033[0m")
...
def axl1():
    ...
    ethan()
    folder_path = "/sdcard/boostphere"
    print(f"     {yellow}[01] {red}FRA EXTRACT ACCOUNT")
    print(f"     {yellow}[02] {red}FRA EXTRACT PAGES")
    print(f"     {yellow}[03] {red}RPW EXTRACT ACCOUNT")
    print(f"     {yellow}[04] {red}RPW EXTRACT PAGES")
    print(f" {Q7}─────────────────────────────────────────────────────────────────\033[0m")
    save_choice = input(f"{white} CHOOSE : ").strip()
    if save_choice == '1':
        account_file = os.path.join(folder_path, "FRAACCOUNT.txt")
...
def perform_reaction_fast():
    ...
    clear_screen()
    ethan()
    print(f"     {yellow}[01] {Q7}FRA ACCOUNT")
    print(f"     {yellow}[02] {Q7}FRA PAGES")
    print(f"     {yellow}[03] {Q7}RPW ACCOUNT")
    print(f"     {yellow}[04] {Q7}RPW PAGES")
    print(f" {Q7}─────────────────────────────────────────────────────────────────\033[0m")
    try:
        file_choice = int(input(f"{white} CHOOSE : ").strip())
        print(f" {Q7}─────────────────────────────────────────────────────────────────\033[0m")
    ...
    uid_url = post_id
    print(f"     {yellow}[01] {blue}LIKE")
    print(f"     {yellow}[02] {red}LOVE")
    print(f"     {yellow}[03] {yellow}WOW")
    print(f"     {yellow}[04] {green}SAD")
    print(f"     {yellow}[05] {red}ANGRY")
    print(f"     {yellow}[06] {blue}HAHA")
    print(f" {Q7}─────────────────────────────────────────────────────────────────\033[0m")
    try:
        reaction_choice = int(input(f"{white} CHOOSE YOUR DESIRE REACTION : ").strip())
        reaction_map = {
            1: "LIKE",
            2: "LOVE",
            3: "WOW",
            ...
            for future in as_completed(future_to_token):
                ...
                try:
                    ...
                    if status_code == 200:
                        reactions_count += 1
                        total_successful_reactions += 1
                        print(f"     {red}[REACTOR ACCOUNT ]  {red}───────> {green}SUCCESSFULLY {red}REACTED!")
                        print(f" {red} WARNING  Fast Reaction May Fast burning ") 
                        print(f" {green} [ {uid} ] [ ACTIVE ]")
                        print(f" {yellow} ( PREMIUM SOFTWARE )")
                    else:
                        pass
                except Exception as e:
                    print(f"Error processing token {token}: {e}")
...
def auto_follow_fast():
    ...
    print(f" {Q7}─────────────────────────────────────────────────────────────────\033[0m")
    tokens = tokens[start_line - 1:]
    account_id = extract_user_id_prof(input(f" {white}ENTER THE TARGET ACCOUNT UID : "))
    print(f" {Q7}─────────────────────────────────────────────────────────────────\033[0m")
    if not account_id:
        print(f"Invalid account ID.")
        return
    try:
        follow_limit = int(input(f' {yellow}TARGET FOLLOWERS: '))
        print(f" {Q7}─────────────────────────────────────────────────────────────────\033[0m")
    except ValueError:
...
def perform_comment_fast():
    ...
    try:
        ...
        return
    tokens = tokens[start_line - 1:]
    a = input(f" {white}ENTER THE POST lINK: ")
        ...
        while successful_comments < num_accounts and tokens:
            ...
            try:
                ...
                if "SUCCESSFULLY" in result:
                    ...
                    print(f"{successful_comments}/{num_accounts} COMMENTS SUCCESSFULLY SENT . ")
            except Exception as e:
                pass
...
def share():
    ...
    print(f"     {yellow}[01] {red}FRA ACCOUNT")
    print(f"     {yellow}[02] {red}FRA PAGES")
    print(f"     {yellow}[03] {red}RPW ACCOUNT")
    print(f"     {yellow}[04] {red}RPW PAGES")
    print(f" {Q7}─────────────────────────────────────────────────────────────────\033[0m")
    choice = input(f"{white} CHOOSE : ").strip()
    ...
    print(f" {Q7}─────────────────────────────────────────────────────────────────\033[0m")
    num_shares = int(input(f" {yellow}TARGET SHARE: "))
    print(f" {Q7}─────────────────────────────────────────────────────────────────\033[0m")
...
def main2(): 
    ...
    clear_screen()
    ethan()
    print(f"""               {Q7}ACCOUNT STORED STATUS 
 {Q7}─────────────────────────────────────────────────────────────────\033[0m
 ...
 {Q7}─────────────────────────────────────────────────────────────────\033[0m""")
    ...
    print(f"     {yellow}[2] {green} AUTOMATIC SHARE")
    print(f" {Q7}─────────────────────────────────────────────────────────────────\033[0m")
    choice = input(f"{white} CHOOSE : ").strip()
    if choice == '1': 
        extraction()
...
def choose_file_path():
    clear_screen()
    ethan()
    print(f"     {yellow}[01] {green}FRA ACCOUNT")
    print(f"     {yellow}[02] {blue}FRA PAGES")
    print(f"     {yellow}[03] {green}RPW ACCOUNT")
    print(f"     {yellow}[04] {blue}RPW PAGES")
    print(f" {Q7}─────────────────────────────────────────────────────────────────\033[0m")
    choice = int(input(f"{white} CHOOSE : ").strip())
    if choice == 0:
...
def check():
    ...
    for line in lines:
        ...
        if val(token):
            ...
            print(f" {red}ACCOUNT ─────> {blue}{uid} {white}= {yellow} IS NOT VALID AND WILL BE REMOVED AUTOMATICALLY ")
    with open(file_path, 'w') as file:
...
def perform_viewfb_fast():
    ...
    print(f" {Q7}─────────────────────────────────────────────────────────────────\033[0m")
    url_str = input(' {white}ENTER PUBLIC Reel/Video URL: ')
    try:
...
def comment_post():
    clear()
    logo()
    user_ids = get_ids_tokens('/sdcard/ERROR-BOOSTING/tokpid.txt')
    access_tokens = get_ids_tokens('/sdcard/ERROR-BOOSTING/tokp.txt')
    print(f"[1] SAME COMMENTS")
    print(f"[2] DIFFERENT COMMENTS")
    linex()
    comment_option = input('CHOOSE : ')
    linex()
    ...
    if post_id is None:
        print(f"{red}COULD NOT EXTRACT POST ID. EXITING.")
        return
    comment_texts = []
    if comment_option == '1':
    ...
    while successful_comments < num_comments:
        ...
        try:
            if not has_commented(post_id, access_token, user_id):
                ...
                response = requests.post(url, params=params)
                if response.status_code == 200:
                    print(f"{green}SUCCESS: COMMENTED ON {post_id} WITH USER ID {user_id}.")
                    successful_comments += 1
                else:
                    print(f"{red}FAILED TO COMMENT USER ID: {user_id} POST ID: {post_id}")
            else:
                print(f"USER {user_id} HAS ALREADY COMMENTED ON THIS POST.")
        except requests.exceptions.RequestException:
            print(f"{red}FAILED TO COMMENT USER ID: {user_id} POST ID: {post_id}")
        current_comment_index += 1
    print(f"{green}TOTAL SUCCESSFUL COMMENTS: {successful_comments}")
...
def clb():
    clear_screen()
    ethan()
    print(f"  please contact the owner to this tool")
    print(f" Main program ")
    print(f" CONTACT KRISTEL MAE ( {blue}FACEBOOK {white})")
...
def ethan():
    print(f"{Q7}Initializing system...\033[0m")
    time.sleep(1)

def count_tokens(file1, file2):
    def count_lines(file_path):
        try:
            ...
            return 0
    return count_lines(file1), count_lines(file2)
...
def show_overview():
    ...
    os.makedirs('/sdcard/boostphere', exist_ok=True)
   ...
   {DD}  ║   {yellow}RPW ACCOUNT{yellow} : {green}{total_account_rpw}                      {DD}    ║
    ...
    {DD} ╠════════════════════════════════════════════╝\033[0m""")
    input("\033[1;37mPress ENTER to continue to the Main  menu...\033[0m")


# ====== MENU SECTION ======
MENU = [
    "ADD FACEBOOK ACCOUNT [ EXTRACTION ] ",
    "AUTOMATIC REACTION [ POST ] ",
    "AUTOMATIC REACTION [ REELS ]",
    "AUTOMATIC REACTION [ GROUP POST ]",
    "AUTOMATIC REACTION [ VIDEO OR PHOTO ]",
    "AUTOMATIC REACTION [ DP OR NORMAL POST ]",
    "AUTOMATIC REACTION [ DP OR COVER ]",
    "AUTOMATIC REACTION [ LOVE / WOW ]",
    "AUTOMATIC REACTION [ ON GOING LIVESTREAM ]",
    "AUTOMATIC CUSTOM COMMENT ",
    "AUTOMATIC CUSTOM COMMENT V2 ",
    "AUTOMATIC COMMENT [ ON GOING LIVESTREAM ] ",
    "AUTOMATIC FOLLOWERS VERSION 1 ",
    "AUTOMATIC FOLLOWERS [ PAGE OR PROFILE ] ",
    "AUTOMATIC SHARE [ BOT ] / FAST SPEED ",
    "AUTOMATIC SHARE [ NOTIFICATION ] FAST ",
    "LIVE ACCOUNT CHECKER [ FETCHES ] ",
    "ACCOUNT TOKEN CHECKER [ AUTOMATIC REMOVER ] ",
    "AUTOMATIC REPORT SYSTEM ( COOKIE )",
    "AUTOMATIC STORY OR VID VIEWS ",
    "AUTOMATIC FILE CREATIONS",
    "AUTOMATIC FB CREATION",
    "AUTOMATIC PAGE CREATION ( THRU COOKIES ) ",
    "TOKEN GETTER ",
    "RESET TOOL",
    "AUTOMATIC SHARE THRU TOKEN",
   "                                                                  ",                     
    "AUTOMATIC IG FOLLOWERS",
    "AUTOMATIC IG LIKES ",
    "AUTOMATIC TIKTOK FOLLOWES",
    "AUTOMATIC TIKTOK VIEWS ",
    "AUTOMATIC MASS REPORT",
]
...
"╚═════╝  ╚═════╝  ╚═════╝ ╚══════╝   ╚═╝   "
    ...
    title = " ⚙️PREMIUM SOFTWARE— TOOL MAIN MENU 🖥️"
    ...
    info = "Use ↑ / ↓ to move, ENTER to select (PC OR ANDROID)"
    stdscr.attron(curses.color_pair(4))
    stdscr.addstr(start_y + len(logo) + 2, max(0, (w - len(info)) // 2), info)
    stdscr.attroff(curses.color_pair(4))

    start_row = start_y + len(logo) + 5
...
# ====== ACTION HANDLER ======
def run_action(stdscr, idx):
    stdscr.clear()
    chosen = MENU[idx]

    if "ADD FACEBOOK ACCOUNT [ EXTRACTION ] " in chosen:
        curses.endwin()
        colorful_loading_bar()
        extraction()
        input("Press Enter to return to menu...")
        return True
    elif "AUTOMATIC REACTION [ POST ]" in chosen:
        curses.endwin()
        perform_reaction_fast()
        input("Press Enter to return to menu...")
        return True
    elif "AUTOMATIC REACTION [ REELS ]" in chosen:
        curses.endwin()
        reels()
        input("Press Enter to return to menu...")
        return True
    elif "AUTOMATIC REACTION [ GROUP POST ]" in chosen:
        curses.endwin()
        perform_reaction_fast()
        input("Press Enter to return to menu...")
        return True
    elif "AUTOMATIC REACTION [ VIDEO OR PHOTO ]" in chosen:
        curses.endwin()
        vid()
        input("Press Enter to return to menu...")
        return True
    elif "AUTOMATIC REACTION [ DP OR NORMAL POST ]" in chosen:
        curses.endwin()
        perform_reaction_fast_dp()
        input("Press Enter to return to menu...")
        return True
    elif "AUTOMATIC REACTION [ DP OR COVER ]" in chosen:
        curses.endwin()
        perform_reaction_fast_cover()
        input("Press Enter to return to menu...")
        return True
    elif "AUTOMATIC REACTION [ LOVE / WOW ]" in chosen:
        curses.endwin()
        perform_reaction_fastt()
        input("Press Enter to return to menu...")
        return True
    elif "AUTOMATIC REACTION [ ON GOING LIVESTREAM ]" in chosen:
        curses.endwin()
        live_react()
        input("Press Enter to return to menu...")
        return True
    elif "AUTOMATIC CUSTOM COMMENT" in chosen:
        curses.endwin()
        perform_comment_fast()
        input("Press Enter to return to menu...")
        return True
    elif "AUTOMATIC CUSTOM COMMENT V2" in chosen:
        curses.endwin()
        comment_post()
        input("Press Enter to return to menu...")
        return True
    elif "AUTOMATIC REPORT SYSTEM ( COOKIE )" in chosen:
        curses.endwin()
        report_fb()
        input("Press Enter to return to menu...")
        return True
    elif "AUTOMATIC COMMENT [ ON GOING LIVESTREAM ]" in chosen:
        curses.endwin()
        live_comment()
        input("Press Enter to return to menu...")
        return True
    elif "AUTOMATIC FOLLOWERS VERSION 1 " in chosen:
        curses.endwin()
        auto_follow_fast()
        input("Press Enter to return to menu...")
        return True
    elif "AUTOMATIC FOLLOWERS [ PAGE OR PROFILE ]" in chosen:
        curses.endwin()
        perform_actions_from_file()
        input("Press Enter to return to menu...")
        return True
    elif "AUTOMATIC SHARE [ BOT ] / FAST SPEED" in chosen:
        curses.endwin()
        main2()
        input("Press Enter to return to menu...")
        return True
    elif "AUTOMATIC SHARE [ NOTIFICATION ] FAST " in chosen:
        curses.endwin()
        pub()
        input("Press Enter to return to menu...")
        return True
    elif "LIVE ACCOUNT CHECKER [ FETCHES ]" in chosen:
        curses.endwin()
        fetch_account_info(file_options)
        input("Press Enter to return to menu...")
        return True
    elif "ACCOUNT TOKEN CHECKER [ AUTOMATIC REMOVER ]" in chosen:
        curses.endwin()
        check()
        input("Press Enter to return to menu...")
        return True        
    elif "RESET TOOL" in chosen:
        curses.endwin()
        clear_text_files()
        input("Press Enter to return to menu...")
        return True
    elif "TOKEN GETTER" in chosen:
        curses.endwin()
        get_token()
        input("Press Enter to return to menu...")
        return True
    elif "AUTOMATIC STORY OR VID VIEWS" in chosen:
        curses.endwin()
        perform_viewfb_fast()
        input("Press Enter to return to menu...")
        return True
    elif "AUTOMATIC FILE CREATIONS" in chosen:
        curses.endwin()
        get_token()
        input("Press Enter to return to menu...")
        return True
    elif "AUTOMATIC FB CREATION" in chosen:
        curses.endwin()
        get_token()
        input("Press Enter to return to menu...")
        return True
    elif "AUTOMATIC SHARE THRU TOKEN" in chosen:
        curses.endwin()
        auto_share()
        input("Press Enter to return to menu...")
        return True
    elif "AUTOMATIC IG FOLLOWERS" in chosen:
        curses.endwin()
        clb()
        input("Press Enter to return to menu...")
        return True
...
# ====== MAIN FUNCTION ======
def main():
    show_overview()  # show stored accounts & pages first
...
def menu_main(stdscr):
    ...
    while True:
        ...
        if key in (curses.KEY_UP, ord('k')):
            ...
            cont = run_action(stdscr, selected_idx)
            if not cont:
                break
